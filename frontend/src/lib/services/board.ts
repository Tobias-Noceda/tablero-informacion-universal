...
export async function create(name: string, owner = "Messi") {
    const res = await api.post("/api/v1/boards", { name, owner });
    return await res.json() as Board;
}

export async function get_all(cognito_id = "Messi") {
    const res = await api.get(`/api/v1/boards?${new URLSearchParams({ cognito_id })}`);
    return await res.json() as Board[];
}
...
export async function get_post_its(id: UUID) {
    const res = await api.get(`/api/v1/boards/${id}/post-its`);
    return await res.json() as PostIt[];
}

export async function share(id: UUID, cognito_id = "Messi") {
    await api.post(`/api/v1/boards/${id}/collaborators`, { cognito_id });
}

// TODO: broken
export async function unshare(id: UUID, cognito_id = "Messi") {
    await api.del(`/api/v1/boards/${id}/collaborators?${new URLSearchParams({ cognito_id })}`);
}

export async function rename(id: UUID, name: string) {
    await api.post(`/api/v1/boards/${id}/name`, { name });
}
